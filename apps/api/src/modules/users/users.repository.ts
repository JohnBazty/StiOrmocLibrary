...
import { isPostgres, sumEquals } from '../../config/sql-dialect.js'
...
export class UsersRepository{
 ...
 async summary(){
  const sql=isPostgres
   ?`SELECT COUNT(*) active_accounts,${sumEquals('role','Student')} student_accounts,${sumEquals('role','Faculty')} faculty_accounts,COUNT(*) FILTER (WHERE role IN ('Admin','Librarian')) staff_accounts FROM accounts WHERE account_status='Active'`
   :`SELECT COUNT(*) active_accounts,SUM(role='Student') student_accounts,SUM(role='Faculty') faculty_accounts,SUM(role IN ('Admin','Librarian')) staff_accounts FROM accounts WHERE account_status='Active'`
  const[rows]=await this.pool.execute<RowDataPacket[]>(sql);return rows[0]
 }
