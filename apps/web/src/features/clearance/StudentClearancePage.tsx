...
export function StudentClearancePage() {
  ...
  return <>
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-2">
      <SectionCard className="overflow-hidden"><div className="border-b border-[#003399]/15 px-5 py-4"><h2 className="font-bold text-[#003399]">Unreturned books</h2></div><div className="divide-y divide-[#003399]/10">{data?.loans.length ? data.loans.map((loan) => <div key={loan.transactionId} className="p-5"><div className="flex justify-between gap-3"><div><p className="font-bold text-[#003399]">{loan.title}</p><p className="mt-1 text-xs text-[#003399]/60">{loan.accessionNumber ?? `Transaction ${loan.transactionId}`}</p></div><StatusBadge status={loan.status} /></div><div className="mt-3 grid grid-cols-2 gap-2 text-xs text-[#003399]/70"><span>Due<br /><strong>{date(loan.dueAt)}</strong></span><span>Overdue<br /><strong>{loan.overdueHours} hours</strong></span></div></div>) : <p className="p-8 text-center text-sm text-[#003399]/65">No unreturned books.</p>}</div></SectionCard>
      <SectionCard className="overflow-hidden"><div className="border-b border-[#003399]/15 px-5 py-4"><h2 className="font-bold text-[#003399]">Outstanding charges</h2></div><div className="divide-y divide-[#003399]/10">{data && (data.fines.length || data.lostBooks.some((item) => item.status === 'Confirmed' && item.paymentStatus === 'Unpaid')) ? <>{data.fines.map((fine) => <div key={`fine-${fine.fineId}`} className="flex justify-between gap-3 p-5"><div><p className="font-bold text-[#003399]">{fine.basis === 'Manual' ? 'Library infraction' : 'Overdue fine'} · {fine.title}</p><p className="mt-1 text-xs text-[#003399]/60">{fine.basis === 'Manual' ? (fine.notes ?? 'Documented library infraction') : `${fine.overdueUnits} ${fine.basis.toLowerCase()} units at ${money(fine.rate)}`}</p></div><strong className="text-[#003399]">{money(fine.amount)}</strong></div>)}{data.lostBooks.filter((item) => item.status === 'Confirmed' && item.paymentStatus === 'Unpaid').map((item) => <div key={`lost-${item.lostBookReportId}`} className="flex justify-between gap-3 p-5"><div><p className="font-bold text-[#003399]">Lost-book replacement · {item.title}</p><p className="mt-1 text-xs text-[#003399]/60">Confirmed {date(item.verifiedAt)}</p></div><strong className="text-[#003399]">{money(item.replacementCharge)}</strong></div>)}</> : <p className="p-8 text-center text-sm text-[#003399]/65">No unpaid charges.</p>}</div>{data ? <div className="flex justify-between bg-[#003399] p-5 text-white"><span className="font-bold">Total outstanding</span><strong>{money(data.summary.totalOutstanding)}</strong></div> : null}</SectionCard>
    </div>
    <SectionCard className="mt-5 p-5"><div className="flex gap-3"><Clock3 className="text-[#003399]" /><div><h3 className="font-bold text-[#003399]">How clearance is determined</h3><p className="mt-1 text-sm leading-6 text-[#003399]/70">Any unreturned book, unpaid overdue fine, or unpaid confirmed lost-book replacement charge blocks clearance. Authorized staff may record a reasoned override, and its full history remains visible for audit.</p></div></div></SectionCard>
  </>
}
