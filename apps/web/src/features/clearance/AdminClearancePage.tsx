...
export function AdminClearancePage() {
  ...
  return <>
    ...
    <SectionCard className="mb-5 overflow-hidden"><div className="border-b border-[#003399]/15 p-5"><h2 className="font-bold text-[#003399]">Lost-book reports awaiting review ({data?.pendingLostReports.length ?? 0})</h2><p className="mt-1 text-xs text-[#003399]/65">Student and faculty reports appear here as soon as they are submitted.</p></div><div className="divide-y divide-[#003399]/10">{!data ? <p className="p-5 text-sm text-[#003399]/65">Loading lost-book reports…</p> : data.pendingLostReports.length ? data.pendingLostReports.map((report) => <div key={report.lostBookReportId} className="flex flex-wrap items-center justify-between gap-3 p-5"><div><p className="font-bold text-[#003399]">{report.title}</p><p className="mt-1 text-sm text-[#003399]/70">{report.borrowerName} · {report.schoolId} · {report.role}</p><p className="mt-1 text-xs text-[#003399]/60">Reported {date(report.reportedAt)}</p></div><Button variant="secondary" onClick={() => void open(report.userId)}><Eye size={16} /> Review report</Button></div>) : <p className="p-5 text-sm text-[#003399]/65">No lost-book reports are waiting for review.</p>}</div></SectionCard>
    ...
    {selected ? <div className="fixed inset-0 z-50 overflow-y-auto bg-[#003399]/75 p-4"><div className="mx-auto my-4 max-w-5xl rounded-2xl bg-white shadow-2xl"><header className="flex items-start justify-between border-b border-[#003399]/15 p-5"><div><p className="text-xs font-bold uppercase text-[#003399]/60">Clearance review</p><h2 className="font-display text-xl font-bold text-[#003399]">{selected.student.name}</h2><p className="text-sm text-[#003399]/60">{selected.student.schoolId} · {selected.student.program}</p></div><button aria-label="Close" onClick={() => setSelected(null)} className="p-2 text-[#003399]"><X /></button></header><div className="space-y-5 p-5"><div className="grid gap-3 sm:grid-cols-4"><StatCard label="Standing" value={selected.status} icon={BadgeCheck} tone="blue" /><StatCard label="Unreturned" value={selected.summary.activeLoans} icon={Clock3} tone="blue" /><StatCard label="Outstanding fines" value={money(selected.summary.unpaidOverdueFines)} icon={Clock3} tone="blue" /><StatCard label="Lost charges" value={money(selected.summary.unpaidReplacementCharges)} icon={Clock3} tone="blue" /></div>
      ...
      {selected.lostBooks.length ? <SectionCard className="p-4"><h3 className="font-bold text-[#003399]">Lost-book reports</h3><div className="mt-3 space-y-3">{selected.lostBooks.map((item) => <div key={item.lostBookReportId} className="rounded-xl border border-[#003399]/15 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><div><strong className="text-[#003399]">{item.title}</strong><p className="text-xs text-[#003399]/60">Reported {date(item.reportedAt)} · {item.status} · {item.paymentStatus}</p></div><strong className="text-[#003399]">{item.status === 'Pending' ? 'Charge pending review' : item.status === 'Rejected' ? 'No charge' : money(item.replacementCharge)}</strong></div><div className="mt-3 flex flex-wrap gap-2">{item.status === 'Pending' ? <><Button disabled={busy} onClick={() => void decide(item.lostBookReportId, 'Confirmed', item.purchasePrice)}>Confirm loss</Button><Button variant="secondary" disabled={busy} onClick={() => void decide(item.lostBookReportId, 'Rejected', item.purchasePrice)}>Reject report</Button></> : null}{item.status === 'Confirmed' && item.paymentStatus === 'Unpaid' ? <Button disabled={busy} onClick={() => void settle(item.lostBookReportId)}>Record replacement payment</Button> : null}</div></div>)}</div></SectionCard> : null}
